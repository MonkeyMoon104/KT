...
public class GlowMissileBlocks {
    ...
    public static void clearBlocks(List<Location> blocks) {
        for (Location loc : blocks) {
            WorldGuardUtils.runWithWorldGuardBypass(loc, () -> loc.getBlock().setType(Material.AIR));
        }
    }

    public static void placeMissileBlocks(Location base, Location top, List<Location> missileBlocks) {
        WorldGuardUtils.runWithWorldGuardBypass(base, () -> base.getBlock().setType(Material.IRON_BLOCK));
        WorldGuardUtils.runWithWorldGuardBypass(top, () -> top.getBlock().setType(Material.IRON_BLOCK));
        ...
        for (Vector offset : Arrays.asList(
            ...
            WorldGuardUtils.runWithWorldGuardBypass(fenceLoc, () -> fenceLoc.getBlock().setType(Material.NETHER_BRICK_FENCE));
