...
public class DamageUtils {
    ...
    public static DamageConfig getDamageConfig(String effectName, Plugin plugin) {
        String configKey = effectName.toLowerCase();
        if (plugin instanceof KT kt) {
            configKey = kt.getEffectConfigKey(effectName);
        }

        ConfigurationSection section = plugin.getConfig().getConfigurationSection("effects." + configKey + ".damage");
